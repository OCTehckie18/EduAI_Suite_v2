...
const navItems = [
  ...
  },
  {
    ...
    label: "Teacher Tools",
    href: "/tools",
    keywords: "question paper generator quiz",
  },
  {
    icon: Gamepad2,
    label: "Game Studio",
    href: "/games",
    keywords: "gamify create play interactive",
  },
  {
    icon: Calendar,
    label: "Appointments",
    href: "/appointments",
    keywords: "bookings approvals office hours",
  },
  {
    icon: BookOpen,
    label: "Classrooms",
    href: "/classrooms",
    keywords: "courses students assignments",
  },
  {
    icon: ClipboardList,
    label: "Exams",
    href: "/exams",
    keywords: "mcq test assessment quiz",
  },
  // { icon: CheckSquare, label: "OMR Grading", href: "/omr", keywords: "grade paper omr image scan answers ai score" },
  {
    ...
    label: "Reports",
    href: "/reports",
    keywords: "export download pdf analytics",
  },
  // { icon: FileSignature, label: "Form Automation", href: "/forms", keywords: "template auto-fill document" },
  {
    ...
    href: "/analytics",
    keywords: "risk alert attendance prediction",
  },
  {
    icon: Mail,
    label: "Mail Students",
    href: "/mail",
    keywords: "email send students parents bulk communication",
  },
  {
    icon: Calendar,
    label: "Calendar",
...
export const DashboardShell: React.FC = () => {
  ...
  const handleAskAnything = useCallback(
    (prompt = searchQuery) => {
      const trimmedPrompt = prompt.trim();
      navigate(
        trimmedPrompt
          ? `/chat?prompt=${encodeURIComponent(trimmedPrompt)}`
          : "/chat",
      );
      setSearchQuery("");
  ...
  return (
    ...
    >
      {/* ── Top App Bar ──────────────────────────────────────── */}
      <header
        className="fixed top-0 left-0 right-0 z-50 h-[64px] flex items-center justify-between px-4"
        style={{
      ...
      >
        ...
        <div className="flex-1 max-w-2xl px-4 hidden md:flex justify-center">
          <div className="relative w-full max-w-[500px]">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleAskAnything();
                if (e.key === "Escape") {
                  setSearchFocused(false);
                  setSearchQuery("");
                  searchRef.current?.blur();
        ...
        <div className="flex items-center gap-3">
          {/* Session Timer */}
          <div
            className="hidden px-2 py-1 bg-red-500/10 text-red-600 rounded-md text-xs font-bold font-mono border border-red-500/20 shadow-sm"
          ...
          <div className="relative">
            ...
            {notifOpen && (
              <div
                className="absolute right-0 top-12 w-80 rounded-xl z-50 overflow-hidden animate-fade-in border shadow-lg"
                style={{
                  background: "var(--color-surface-base)",
                  borderColor: "var(--color-border)",
                }}
              >
                ...
                <div
                  className="divide-y max-h-[400px] overflow-y-auto custom-scrollbar"
                  style={{ borderColor: "var(--color-border)" }}
                >
          ...
          <div className="relative">
            ...
            {profileOpen && (
              <div
                className="absolute right-0 top-12 w-52 rounded-xl z-50 overflow-hidden animate-fade-in py-1 border shadow-lg"
                style={{
                  background: "var(--color-surface-base)",
                  borderColor: "var(--color-border)",
                }}
              >
                ...
                >
                  <p
                    ...
                    style={{ color: "var(--color-text-primary)" }}
                  >
                    {user?.name || "EduAI User"}
                  </p>
                  <p
                    className="text-[11px] mt-0.5"
                    style={{ color: "var(--color-text-secondary)" }}
                  >
                    {user?.email || "user@eduai.com"}
                  </p>
                </div>
                <div
      ...
      {/* ── Sidebar Drawer ───────────────────────────────────── */}
      <aside
        className={`fixed top-[64px] bottom-0 left-0 z-40 w-[260px] flex flex-col transition-transform duration-300 transform ${
        ...
        <nav className="flex-1 overflow-y-auto py-4 pr-3 space-y-1 custom-scrollbar">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }: { isActive: boolean }) =>
                `flex items-center gap-4 px-5 py-3.5 rounded-r-full transition-all group ${
                  isActive
          ...
          {user?.role === "admin" && (
            <NavLink
              ...
              className={({ isActive }: { isActive: boolean }) =>
                `flex items-center gap-4 px-5 py-3.5 rounded-r-full transition-all group mt-4 ${
                  isActive
      ...
      {/* ── Main Content ─────────────────────────────────────── */}
      <main
        className={`flex-1 transition-all duration-300 pt-[64px] ${sidebarOpen ? "md:ml-[260px]" : "ml-0"}`}
      >
        <div className="p-6">
          <Outlet />
        </div>
      ...
      {toast && (
        <div className="fixed top-20 right-6 z-[100] animate-slide-in-right">
          <div
            className="glass-card p-4 flex items-start gap-4 shadow-2xl border-l-4 border-l-[#264796] min-w-[300px] cursor-pointer"
            onClick={() => {
              navigate(toast.href);
              setToast(null);
            }}
